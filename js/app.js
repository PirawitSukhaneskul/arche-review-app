...
function onRoute(next) {
  ...
  if (route.view === 'feedback') {
    ...
    show('feedback');
    setHeader({ sheet: 'FB-01', title: 'ความเห็นลูกค้า / Client feedback' });
    renderFeedback(views.feedback, data);
    return;
  }

  // Fall back to the first ready meeting when the hash names none (or a bad one).
  const meeting = findMeeting(data, route.meetingId)
    ?? data.meetings.find((m) => m.status === 'ready')
    ?? data.meetings[0];
